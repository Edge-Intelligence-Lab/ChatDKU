...
```bash
nohup python -u -m http.server 9017 -d chatdku/frontend > ./logs/python_server_logs.txt &
disown -h
```
...
Next, start the `agent_app.py` service. This is the agent interface. (agent_app use port `9018` now)
```bash
...
Port, number of workers, and timeout can be adjusted. __Use port `9018` to match frontend's setting.__

```bash
nohup python -u -m gunicorn chatdku.backend.agent_app_parellel:app --bind 0.0.0.0:9018 --workers 32 --timeout 120 > ./logs/agent_logs.txt &
disown -h
```

### Feedback Collection Backend (`save_feedback.py`)

Finally, start the `save_feedback.py` service. (Using port 9019 now)
```bash
