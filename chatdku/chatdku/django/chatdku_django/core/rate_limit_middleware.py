...
class RateLimitMiddleware:
    """
    Rate limiting middleware using IP-based identification.
    """
    ...
    def extract_identifier(self, request):
        """
    ...
    def check_rate_limit(self, identifier, path, limit_type):
        """
        ...
        # Generate cache key
        cache_key = f'ratelimit:{identifier}:{path}:{limit_type}:{window_key}'
    ...
    def __call__(self, request):
        ...
        # 2. Extract client identifier (IP address)
        ...
        # 4. Check rate limit
        allowed, retry_after = self.check_rate_limit(identifier, request.path, limit_type)

        if not allowed:
            # Log rate limit event
            self.logger.warning(
                f"Rate limit exceeded: ip={identifier}, "
        ...
        cache_key = f'ratelimit:{identifier}:{request.path}:{limit_type}:{window_key}'
        current_count = cache.get(cache_key, 0)
