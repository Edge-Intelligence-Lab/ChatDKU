...
from chat.models import ChatMessages, UserSession
from chat.serializer import (
    ChatMessageSerializer,
    FeedbackSerializer,
    SessionSerializer,
    SessionVerifierSerializer,
    SourceSerializer,
)
from chat.utils import load_conversation, title_gen
        ...
        request={
            "application/json": {
                ...
                "properties": {
                    ...
                    "chatHistoryId": {"type": "string", "format": "uuid"},
                },
                "required": ["mode", "messages", "chatHistoryId"],
            }
        },
        responses={200: OpenApiResponse(response={"type": "string"})},
        examples=[
            OpenApiExample(
                "Request Example",
                value={
...
class ChatView(APIView):
    ...
    def post(self, request):
        ...
        search_mode, docs = (
            source_serializer.validated_data["search_mode"],
            source_serializer.validated_data["docs"],
        )
        netid = request.netid
        ...
        try:
            ...
            chat_serializer = ChatMessageSerializer(
                data={"role": ChatMessages.USER, "message": message_content}
            )
            ...
            if not session.title:
                ...
                try:
                    ...
                    title = loop.run_until_complete(
                        title_gen(message_content)
                    )  # Async to prevent further latency
            ...
            if not session.title:
                UserSession.objects.filter(id=session.id, title="").update(title=title)
                ...
                try:
                    ...
                    if response_text:
                        ...
                        bot_serializer = ChatMessageSerializer(
                            data={"role": ChatMessages.BOT, "message": response_text}
                        )
...
class SessionViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(description="All the session_id for a user", parameters=PARAMETERS)
    def get_queryset(self):
        return (
            UserSession.objects.filter(Q(user=self.request.user))
            .exclude(Q(title="") | Q(title__isnull=True))
            .order_by("-created_at")
        )
    ...
    def rename(self, request, pk=None):
        ...
        return Response({"id": session.id, "title": session.title}, status=200)
