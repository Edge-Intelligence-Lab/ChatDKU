...
export function ComboBoxResponsive({ inputValue, onEndpointChange }: ComboBoxResponsiveProps) {
	...
	const [selectedStatus, setSelectedStatus] = React.useState<Branch | null>(branches[0]);
...
function StatusList({ setOpen, setSelectedStatus }: { setOpen: (open: boolean) => void; setSelectedStatus: (status: Branch | null) => void }) {
	return (
		<Command>
				...
				<CommandGroup>
					{branches.map((status) => (
						<CommandItem
							...
							onSelect={(value) => {
								setSelectedStatus(branches.find((priority) => priority.chatEndpoint === value) || null);
								setOpen(false);
							}}
						>
							{status.label}
