...
def main():
    ...
    contexts, reranked_nodes = rag_tools.llamaindex_tool.query(
        query, retriever="fusion"
    )
    memory.contexts.extend(contexts)
    memory.llamaindex_nodes.extend(reranked_nodes)

    #### if choose graphrag
    contexts_list, retrieved_reports, retrieved_entities = (
        rag_tools.graphrag_tool.global_query(query)
    )
