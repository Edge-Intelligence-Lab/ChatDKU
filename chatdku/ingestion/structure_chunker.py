    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_num, page in enumerate(pdf.pages):
            ...
            for line in lines:
                ...
                # Detect font size for this specific line
...
def detect_title_level(text: str, font_size: float, default_size: float) -> int:
    ...
    # Rule 1: All caps with reasonable length (likely a main heading)
...
def split_items_by_level(
    ...
    Args:
        ...
        max_chunk_size: Maximum chunk size in characters (reserved)
    ...
    for item in items:
        ...
        # When we hit the target heading level, start a new chunk
...
def split_text_by_sentences(
    ...
    for line in text_lines:
        ...
        # Handle extra-long lines that exceed max_chunk_size by themselves
        if line_size > max_chunk_size:
            ...
            # Normal line - add to current chunk if it fits
...
def structure_aware_chunking(
    ...
    Returns:
        List of chunks, each chunk is a list of text lines
    """
    # First pass: split by level-1 headings
    temp_list = []
    chunked_list = []
    prev_j = None
            ...
            if len(temp_list) > 0:
                ...
                if len(temp_text) > max_chunk_size:
                    ...
                    sub_chunks = split_items_by_level(
                        items[prev_j:j], 2, max_chunk_size
                    )
...
def process_pdf(
    ...
    Args:
        pdf_path: Path to the PDF file
        max_chunk_size: Maximum chunk size in characters (default: 512)
