...
class FeedbackSignature(dspy.Signature):
    ...
    feedback_text: str = dspy.InputField(
        desc="A corpus of feedback dating from last 30 days"
    )

    summary: str = dspy.OutputField(
        desc="A summary of all the Feedback, including the most frequently occuring, beginning with <answer> and ending with </answer>"
    )
    evidence: str = dspy.OutputField(
        desc="A short evidence for frequently occuring feedback."
    )
...
def load_weekly_data():
    try:
        ...
        stats["failure_percentage"] = (stats["Failure Count"] * 100) / stats[
            "Request Count"
        ]
...
def ping_lm(message: str):
    response = client.chat.completions.create(
        model=config.llm,
        messages=[
            {"role": "system", "content": "This is a ping test."},
