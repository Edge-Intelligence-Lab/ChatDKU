...
class ResponseLengthError(Exception):
    def __init__(self, length, min_length=100, *args):
        ...
        super().__init__(
            f"The length of Response is less than the min-length: {self.min_length}. Length: {self.length}. Other information: {args[0]}"
        )
...
class MyUser(HttpUser):
    ...
    def get_doc_list(self):
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse document list: {e}. Raw response: {response.text}"
            )
            return []
    ...
    def post_chat(self):
        ...
        try:
            ...
            logger.info(
                f"POST /dev/django/chat | Status: {response.status_code} | Response: {len(message)}\n"
            )
        except Exception as e:
