...
class Request(db.Model):
    ...
    date_: so.Mapped[datetime] = so.mapped_column(
        sa.DateTime, primary_key=True, unique=True
    )
    ...
    @classmethod
    def get_date_count(
        cls, startdate: date | None = None, enddate: date | None = None
    ) -> int:
        ...
        dates = (
            sa.select(date_only, sa.func.sum(cls.req_count))
            .where(cls.date_.between(start_date, end_date))
            .group_by(date_only)
            .order_by(date_only)
        )
