...
Access the Prometheus web interface at `http://10.200.14.82:9090` to:
...
1. **Check Prometheus UI**: Navigate to `http://10.200.14.82:9090`
...
sudo docker run -d \                     
  ...
  --redis.user=default \            
  --redis.password=YPpVXycZ6xsjCLHLnaSC
```
You might need to disown the process after starting it.
