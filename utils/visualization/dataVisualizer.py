...
class DataVisualizer:
    ...
    def plot_2d_distribution(
        self, x_col, y_col, kind="scatter", bins=30, kde=True, cmap="viridis"
    ):
        """
        ...
        if kind == "scatter":
            ...
            if kde:
                sns.kdeplot(
                    data=self.data, x=x_col, y=y_col, levels=5, color="red", alpha=0.6
                )
    ...
    def plot_3d_distribution(self, x_col, y_col, z_col, kind="scatter", cmap="viridis"):
        ...
        if kind == "scatter":
            ...
            Z = np.sin(X) * np.cos(
                Y
            )  # Example; replace with your own data interpolation logic
